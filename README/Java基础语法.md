...
![关键字表](https://canvs.oss-cn-chengdu.aliyuncs.com/canvs_typora/%E5%85%B3%E9%94%AE%E5%AD%97%E8%A1%A8.png)
...
- 值
  - 基本数据类型的变量：存储数据值
  - 引用数据类型的变量：存储地址值，即对象的首地址。例如：String类型的变量存储的是字符串对象的收地址
...
在Java程序中，不同的基本数据类型的值经常需要进行互相转换。Java语言所提供的**七种数据类型**之间可以相互安转换，基本数据类型转换有两种方式：自动类型转换和强制类型转换。boolean类型不参与。
